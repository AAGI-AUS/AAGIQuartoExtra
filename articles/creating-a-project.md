...
This article provides a guide on using the functions in the
AAGIQuartoExtra package to create a structured project, manage folder
organization, and work with Quarto documents effectively. There are five
important steps we want to cover:

1.  Create a new project.
2.  Set up a structured folder hierarchy using
    [`setup_folders()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/setup_folders.md).
3.  Manage and organize Quarto reporting documents in the project with
    [`setup_docs()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/setup_docs.md).
4.  Use
    [`download_fonts()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/download_fonts.md)
    to obtain the Proxima Nova font and understand fallback mechanisms.
...
![](../reference/figures/create_project_step2.png)
...
![](../reference/figures/create_project_step3.png)
...
![](../reference/figures/create_project_step4.png)

If everything is set up correctly, you should be able to work and render
AAGI Quarto reports in the created directory. Below, we will cover some
additional functionalities that can help and facilitate the organisation
of the project files.

## 2. Setting up a folder structure

When learning data analysis, it can be challenging to figure out how to
organise files within the created project. It is good practice to
establish a comprehensive file organisation structure that helps us to
navigate and re-use the project content without losing a huge amount of
time searching across lines of codes and files.
...
[`setup_folders()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/setup_folders.md)
function can help create a standardized folder hierarchy. This function
organizes your project with the following folders:
...
- **admin**: For administrative files like project timelines or meeting
  notes.
...
You will see that the function will create the directories in your
project root. You are free to remove/modify/add folders depending on
...
For instance, let’s say we are working on a Wheat Nitrogen Response
...
render the document to PDF and Word formats and save them in the `/doc`
directory as `01_WheatNitrogenResponse_Report.pdf` and
`01_WheatNitrogenResponse_Report.docx`. If everything is set up
correctly, you should see rendered PDF and Word documents similar to
these:

![](../reference/figures/create_project_step6.png)

You should try to follow the naming convention for additional documents
within the same project, for instance:

- `02_WheatNitrogenResponse_Manuscript.qmd`
- `03_WheatNitrogenResponse_ConferencePaper.qmd`
- `04_WheatNitrogenResponse_Slides.qmd`

The numbering system ensures clarity and logical ordering. Another
possibility/recommendation is to include version number or dates written
...
The template uses the Proxima Nova font if available. If the font is not
installed and you render any document, the package will fall back to
Arial font.
...
AAGI repository, you can use the
[`download_fonts()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/download_fonts.md)
function to retrieve and add them your project. This function:

1.  Downloads the required `.otf` font files from the private GitHub
    repository.
2.  Extracts the files into the `_extensions/aagi/fonts/` directory
    within your project.

While doing that, the function will automatically call
[`setup_github_pat()`](https://aagi-aus.github.io/AAGIQuartoExtra/reference/setup_github_pat.md)
to ensure you have the correct permissions to download from the
repository. Run the following command to download the fonts:

``` r
AAGIQuartoExtra::download_fonts()
```

If the fonts are downloaded successfully, the template will from now use
Proxima Nova. If the download fails or the font is unavailable, Arial
will continue to be used as the fallback.
...
your project using the `usethis` package. To configure Git for your
project:
...
This will create a new private repository on GitHub and link it to your
local project.

For detailed steps, I leave you here with a great guide on using Git in
R written by Jennifer Bryan: [Happy Git with R
guide](https://happygitwithr.com/existing-github-last.html).

## Conclusion

This vignette has shown you how to create and structure a new project
using `AAGIQuartoExtra`. For more details, consult the package
documentation.

Further work to improve functionality will involve:

- Adding support for other AAGIQuarto templates, such as short reports
  and presentations.
